function varargout = MPanEnvelope(NAME, TSTOP, MEMVARS, varargin)
% MPanEnvelope runs a PAN envelope. A netlist must be already loaded
% with MPanNetLoad.
%
% Usage: MPanEnvelope(NAME, TSTOP)
%        MPanEnvelope(NAME, TSTOP, [], varargin)
%        S = MPanEnvelope(NAME, TSTOP, MEMVARS, varargin)
%
% MPanEnvelope(NAME, TSTOP) runs a PAN envelope whose identifier is
% NAME. The simulation is perfomed up to TSTOP. The default options are
% used. 
%
% MPanEnvelope(NAME, TSTOP, [], varargin) runs a PAN envelope analysis whose
...
%
% S = MPanEnvelope(NAME, TSTOP, MEMVARS, varargin) works as the previous one
...
if nargin > 2
    ...
    if rem(nargin,2)  == 0
        error('Beside NAME, STOP, and MEMVARS an even number of inputs is expected')
    end
end

str_command = [NAME ' envelope tstop = ' num2str(TSTOP,'%23.16e')];
