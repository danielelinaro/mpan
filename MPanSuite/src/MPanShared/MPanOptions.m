function options = MPanOptions(KeyNames, varargin)
...
for i = 1:np
  ...
  if ~ischar(key)
    error('MPanSuiteError: Argument %d is not a string property name.', j);
  end
  
  % Get the index in keynames that exactly matches the current key
  % (modulo the case)
    ik = strcmpi(KeyNames, key);
  if ~any(ik)
    error('MPanSuiteError: Unrecognized property %s', key);
  end
